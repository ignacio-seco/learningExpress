...
router.post("/new/:animalId", async (request, response) => {
  try {
    ...
    await relationModel.findByIdAndUpdate(animalId,{$push:{dadosCruzamentos:newData._id}})
...
router.delete("/delete/:id", async (request, response) => {
  try {
    ...
    await relationModel.findByIdAndUpdate(deleteData.animal,{$pull:{dadosCruzamentos:id}})
